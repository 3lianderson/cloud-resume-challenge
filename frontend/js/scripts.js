...
window.addEventListener('DOMContentLoaded', event => {
    ...
    fetch('https://zmaqt6qtlvlrdcbahzhz5oglku0ezwlp.lambda-url.us-east-1.on.aws/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ tableName: "web_visits" })
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        return fetch('https://paawe64qddzoew2xxyrysbeju40entei.lambda-url.us-east-1.on.aws/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
